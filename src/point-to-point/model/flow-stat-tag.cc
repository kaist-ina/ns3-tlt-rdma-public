...
namespace ns3 {
	...
	{
		NS_ASSERT(ttl == FLOW_END || ttl == FLOW_NOTEND || ttl == FLOW_START || ttl == FLOW_START_AND_END || ttl == FLOW_FIN);
		flow_stat = ttl;
	}
	...
	void FlowStatTag::setInitiatedTime(double t) {
		this->initiatedTime = t;
	}

	double FlowStatTag::getInitiatedTime() {
